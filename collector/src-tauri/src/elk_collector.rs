...
mod tests {
    ...
    fn test_collector(max_hits_per_trace: usize) -> ElkCollector {
        ElkCollector {
            config: ElkConfig {
    ...
    #[test]
    fn test_build_exact_trace_queries_creates_one_body_per_trace() {
        let collector = test_collector(123);
        let window = TimeWindow {
            start: "2026-06-23T10:00:00+08:00".into(),
            end: "2026-06-23T10:05:00+08:00".into(),
        };
        let trace_ids = vec!["trace-a".to_string(), "trace-b".to_string()];

        let bodies = collector.build_exact_trace_queries(&trace_ids, &window);

        assert_eq!(bodies.len(), 2);
        assert_eq!(bodies[0]["size"], 123);
        assert_eq!(
            bodies[0]["query"]["bool"]["must"][0]["term"]["traceId"],
            "trace-a"
        );
        assert_eq!(bodies[1]["size"], 123);
        assert_eq!(
            bodies[1]["query"]["bool"]["must"][0]["term"]["traceId"],
            "trace-b"
        );
    }
}
