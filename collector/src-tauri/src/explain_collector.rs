...
mod tests {
    ...
    fn test_should_explain_slow_sql() {
        ...
        assert!(!should_explain(500.0, 500.0));
    }

    #[test]
    fn test_max_explain_count() {
        let slow_sqls: Vec<SlowSqlItem> = (0..25)
            .map(|i| SlowSqlItem {
                trace_id: None,
                database_type: "mysql".into(),
                service: None,
                sql_fingerprint: format!("SELECT * FROM table_{}", i),
                duration_ms: 1000.0,
                tables: vec![],
                operation: None,
                rows_examined: None,
        ...
        let candidates: Vec<&SlowSqlItem> = slow_sqls
            .iter()
            .filter(|s| should_explain(s.duration_ms, 500.0))
            .take(MAX_EXPLAIN_COUNT)
            .collect();
    ...
    #[test]
    fn test_build_executable_sql() {
        let sql = "select id from t where uid = ?";
        let exec = build_executable_sql(sql, Some("218713736305705076(String)"));
        assert_eq!(exec, "select id from t where uid = '218713736305705076'");

        let sql2 = "select id from t where age = ?";
        let exec2 = build_executable_sql(sql2, Some("18(Integer)"));
        assert_eq!(exec2, "select id from t where age = 18");

        let sql3 = "select id from t where col = ?";
        let exec3 = build_executable_sql(sql3, None);
        assert_eq!(exec3, sql3);
    }

    #[test]
    fn test_prepare_explain_sql_rejects_unresolved_placeholders() {
        let err = prepare_explain_sql(
            "SELECT count(0) FROM `tb_name_list` WHERE `TYPE` = ?",
            None,
            "postgresql",
        )
        .unwrap_err();

        assert!(err.contains("参数未完整拼装"), "实际错误: {}", err);
    }

    #[test]
    fn test_prepare_explain_sql_converts_mysql_backticks_for_postgresql() {
        let sql = prepare_explain_sql(
            "SELECT count(0) FROM `tb_name_list` WHERE `TYPE` = ?",
            Some("1(Integer)"),
            "postgresql",
        )
        ...
        assert_eq!(
            sql,
            "SELECT count(0) FROM \"tb_name_list\" WHERE \"TYPE\" = 1"
        );
    }

    #[test]
    fn test_merge_schema_candidates_preserves_selected_and_adds_discovered() {
        let merged = merge_schema_candidates(
            &["public".to_string(), "outbound_platform".to_string()],
            vec![
                "outbound_platform".to_string(),
                "audit_platform".to_string(),
            ],
        );
    ...
    #[test]
    fn test_quote_pg_ident_safe() {
        assert_eq!(quote_pg_ident("pcm_2912"), "pcm_2912");
        assert_eq!(quote_pg_ident("outbound_platform"), "outbound_platform");
        assert_eq!(quote_pg_ident("public"), "public");
    }

    #[test]
    fn test_quote_pg_ident_unsafe() {
        // 大写、含连字符、含空格 → 双引号包裹
        assert_eq!(quote_pg_ident("PCM_2912"), "\"PCM_2912\"");
        assert_eq!(quote_pg_ident("my-schema"), "\"my-schema\"");
        assert_eq!(quote_pg_ident("with space"), "\"with space\"");
        // 含双引号 → 转义
        assert_eq!(quote_pg_ident("a\"b"), "\"a\"\"b\"");
        // 数字开头 → 双引号包裹
        assert_eq!(quote_pg_ident("2912_test"), "\"2912_test\"");
    }
}
