...
mod tests {
    ...
    #[test]
    fn test_resolve_pcm_management_url() {
        let url = "http://172.29.60.151/gateway/pcm-management/v1/pt/speech-module/list?pageNum=1&pageSize=10";
        let resolved = resolve_url(url, "/gateway").unwrap();

        assert_eq!(resolved.host, "172.29.60.151");
        assert_eq!(resolved.gateway_prefix, "/gateway");
        assert_eq!(resolved.service, "pcm-management");
        assert_eq!(resolved.api_path, "/v1/pt/speech-module/list");
        assert_eq!(resolved.operation.as_deref(), Some("list"));
        assert_eq!(resolved.resource.as_deref(), Some("speech-module"));
    }

    #[test]
    fn test_resolve_pcm_user_url() {
        let url = "http://172.29.60.151/gateway/pcm-user/v1/user/detail?userId=123";
        let resolved = resolve_url(url, "/gateway").unwrap();

        assert_eq!(resolved.service, "pcm-user");
        assert_eq!(resolved.api_path, "/v1/user/detail");
    }

    #[test]
    fn test_resolve_pcm_followup_url() {
        let url = "http://10.0.1.100/gateway/pcm-followup/v1/followup/task/create";
        let resolved = resolve_url(url, "/gateway").unwrap();

        assert_eq!(resolved.service, "pcm-followup");
        assert_eq!(resolved.api_path, "/v1/followup/task/create");
        assert_eq!(resolved.operation.as_deref(), Some("create"));
    }

    #[test]
    fn test_known_services() {
        assert!(is_known_service("pcm-management"));
        assert!(is_known_service("pcm-user"));
        assert!(!is_known_service("unknown-service"));
        assert_eq!(service_display_name("pcm-management"), "业务管理服务");
    }
}
